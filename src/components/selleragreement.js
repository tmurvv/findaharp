...
function SellerAgreement(props) {
    ...
    async function handleClick() {
        ...
        if (document.querySelector('#agreeTerms').checked&&document.querySelector('#agreeSchedule').checked) {
            ...
            const updatedUser = { 
                ...user, 
                agreements: ['agreed', Date.now(), TERMS_CONDITIONS, `<p style={{borderBottom: '1px solid lightgrey', paddingBottom: '20px'}}><input type='checkbox' id='agreeTerms'/> I, a representative of Vavra Harp, agree to the Findaharp.com <span  style={{color: 'blue', textDecoration: 'underline', cursor: 'pointer'}}onClick={()=>setOpenTerms(true)}>Terms and Conditions</span>.</p>
